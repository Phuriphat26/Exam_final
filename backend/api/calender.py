...
import secrets # สำหรับสร้าง ID จำลอง
import math
...
import os
import traceback # สำหรับ logging error
...
def time_to_minutes(time_str):
    """
    แปลงเวลา "HH:MM" เป็นนาที (เช่น "09:00" -> 540)
    """
    try:
...
def get_user_subjects():
    # (สมมติว่า session "user_id" มาจาก endpoint อื่น)
...
def add_exam_plan():
    ...
    try:
        ...
        study_plan_slots_raw = data["studyPlan"] # นี่คือข้อมูลดิบจาก React
        if not isinstance(study_plan_slots_raw, list) or not study_plan_slots_raw:
            return jsonify({"message": "กรุณาระบุวันเตรียมตัวสอบอย่างน้อย 1 วัน"}), 400

        # --- (Merge Point) ---
        # (ตรรกะ "ซอยย่อย" เวลา)
        available_time_slots = [] # นี่คือ list ของ slot 1 ชั่วโมง
        ...
        if not available_time_slots:
             return jsonify({"message": "ช่วงเวลาที่เลือกไม่เพียงพอสำหรับการจัดตาราง (ขั้นต่ำ 1 ชั่วโมง)"}), 400

        print(f"[INFO] Total slots calculated: {len(available_time_slots)}")
        ...
        new_plan = {
            ...
            "subjects": exam_subjects, # บันทึกวิชาและ priority ที่ใช้
            "exam_date": data["examDate"],
            "study_plan_raw": study_plan_slots_raw, # บันทึกข้อมูลดิบที่ user กรอก
            "generated_schedule": scheduled_plan, # บันทึกตารางที่สร้างได้
        ...
        return jsonify({
            ...
            "generatedSchedule": scheduled_plan # ส่งตารางกลับไปให้ React
        }), 201
